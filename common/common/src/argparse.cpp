...
static string str_format(const string& str, size_t pre, size_t len, bool running_text = true,
                         bool indent_first = true) {
  string s = str;
  stringstream ss;
  string p;
  len -= 2;  // Python seems to not use full length
  if (running_text) replace(s.begin(), s.end(), '\n', ' ');
...
const Option& OptionParser::lookup_long_opt(const string& opt) const {
  ...
  for (optMap::const_iterator it = _optmap_l.begin(); it != _optmap_l.end(); ++it) {
    if (it->first.compare(0, opt.length(), opt) == 0) {
      ...
      if (it->first.length() == opt.length()) break;
    }
  }
  if (matching.size() > 1) {
...
void OptionParser::handle_long_opt(const string& optstr) {
  ...
  if (option._nargs == 1 and value == "") error("--" + opt + " " + _("option requires an argument"));
...
Values& OptionParser::parse_args(const vector<string>& v) {
  ...
  for (list<Option>::const_iterator it = _opts.begin(); it != _opts.end(); ++it) {
    if (it->get_default() != "" and not _values.is_set(it->dest())) _values[it->dest()] = it->get_default();
  }
...
string Option::check_type(const string& opt, const string& val) const {
  ...
  if (type() == "int" || type() == "long") {
    long t;
    if (not(ss >> t)) err << _("option") << " " << opt << ": " << _("invalid integer value") << ": '" << val << "'";
    ...
    complex<double> t;
    if (not(ss >> t)) err << _("option") << " " << opt << ": " << _("invalid complex value") << ": '" << val << "'";
  }
...
Option& Option::action(const string& a) {
  _action = a;
  if (a == "store_const" || a == "store_true" || a == "store_false" || a == "append_const" || a == "count" ||
      a == "help" || a == "version") {
    nargs(0);
  } else if (a == "callback") {
    nargs(0);
