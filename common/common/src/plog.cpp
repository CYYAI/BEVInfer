...
#elif defined(U_OS_WINDOWS)
  ALOG_WARN("LastModify has not support on windows os");
  return 0;
#endif
}
    ...
    if ((*iter_ptr == '/' and iter_ptr not_eq dir_ptr) or *iter_ptr == 0) {
      ...
      *iter_ptr = 0;
      if (!exists(dir_ptr)) {
        if (!mkdir(dir_ptr)) {
          if (!exists(dir_ptr)) {
            ALOG_ERROR("mkdirs %s return false.", dir_ptr);
            return false;
          }
        }
      }
...
#elif defined(U_OS_WINDOWS)
  ALOG_WARN("is_file has not support on windows os");
  return 0;
#endif
}
...
std::string joinName(const std::string& base, const std::string& path) {
  ...
  return base + BEVINFER_CONCAT_SEP + path;
}
...
static void signal_callback_handler(int signum) {
  ALOG_INFO("Capture interrupt signal.");
