#ifndef _BEVINFER_COMMON_ARGPARSE_H_
#define _BEVINFER_COMMON_ARGPARSE_H_
...
class OptionContainer {
 public:
  ...
  virtual OptionContainer& description(const std::string& d) {
    _description = d;
    return *this;
  }
...
class OptionParser : public OptionContainer {
 public:
  ...
  std::vector<std::string> args() { return std::vector<std::string>(_leftover.begin(), _leftover.end()); }
...
class Option {
 public:
  Option(const OptionParser& p) : _parser(p), _action("store"), _type("string"), _nargs(1), _callback(0) {}
  virtual ~Option() {}
...
#endif  // _BEVINFER_COMMON_ARGPARSE_H_
