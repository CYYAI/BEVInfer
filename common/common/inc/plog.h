#ifndef _BEVINFER_COMMON_PLOG_H_
#define _BEVINFER_COMMON_PLOG_H_
...
#define BEVINFER_CONCAT_SEP "_"

#define ALOG_DEBUG(...)                            \
  bevinfer::common::__log_func(__FILE__, __LINE__, \
                               bevinfer::common::LogLevel::Debug, __VA_ARGS__)
#define ALOG_VERBOSE(...)       \
  bevinfer::common::__log_func( \
      __FILE__, __LINE__, bevinfer::common::LogLevel::Verbose, __VA_ARGS__)
#define ALOG_INFO(...)                             \
  bevinfer::common::__log_func(__FILE__, __LINE__, \
                               bevinfer::common::LogLevel::Info, __VA_ARGS__)
#define ALOG_WARN(...)          \
  bevinfer::common::__log_func( \
      __FILE__, __LINE__, bevinfer::common::LogLevel::Warning, __VA_ARGS__)
#define ALOG_ERROR(...)                            \
  bevinfer::common::__log_func(__FILE__, __LINE__, \
                               bevinfer::common::LogLevel::Error, __VA_ARGS__)
#define ALOG_FATAL(...)                            \
  bevinfer::common::__log_func(__FILE__, __LINE__, \
                               bevinfer::common::LogLevel::Fatal, __VA_ARGS__)
#define ALOG_ASSERT(x)                      \
  {                                         \
    int res = (x);                          \
    if (!res) {                             \
      ALOG_ERROR("Error: assert failed\n"); \
      assert(res);                          \
    }                                       \
  }
...
std::string joinName(const std::string& base, const std::string& path);
...
#endif  // _BEVINFER_COMMON_PLOG_H_